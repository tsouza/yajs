// Issue #89 Claim B prototype: a copy of the real src/main/lib/context/
// StreamContext.ts (unmodified engine logic - same ObjectDispatcher,
// PathOperator, YAJSPath, StreamPosition - only relative import paths
// changed to reach them from this spike directory), with exactly one
// addition: an opt-in `skipMatchWhileCapturing` constructor flag that gates
// the `this.match(value)` calls in doOnValue()/startArray() so they are
// skipped for as long as `this.dispatcher` is already active.
//
// This is NOT a correct general implementation of `outermost` mode (it
// would also wrongly suppress legitimate innermost/nested matches - that's
// fine, this file exists ONLY to benchmark the raw hot-path cost of the one
// specific mechanism issue #89's Claim B is about: "once a capture starts,
// no new match can legally start inside it in outermost mode, so
// YAJSPath.match() can be skipped entirely for the rest of that subtree."
// Skipping unconditionally while `this.dispatcher` is truthy is exactly
// that skip, applied indiscriminately - the correct real implementation
// would only apply it when the active capture is actually in outermost
// mode, which costs nothing extra once already gated on `this.dispatcher`
// truthiness, so this is a faithful lower-bound-of-effort proxy for the
// real skip's per-event cost saving.
//
// See spike-89-outermost-innermost/claim-b-bench.ts for how this is driven
// and spike-89-outermost-innermost/ for the rest of the issue #89 spike.
...
export class StreamContextSkipMatch {

    // When true, doOnValue()/startArray()'s match() calls are skipped
    // entirely for as long as a dispatcher is already active - see the file
    // header comment for exactly what this is (and isn't) measuring.
    private readonly skipMatchWhileCapturing: boolean;
    ...
    constructor(path: YAJSPath, onMatch: (path: string[], value?: any) => void,
                ...
                skipMatchWhileCapturing: boolean = false) {
        this.path = path;
        this.onErrorListener = onError;
        this.pathIncludeArrayIndex = pathIncludeArrayIndex;
        this.skipMatchWhileCapturing = skipMatchWhileCapturing;
    ...
    doOnValue(value?: any): void {
        // Issue #89 Claim B: while a dispatcher is already active, this
        // event is inside the span of a capture that already started - in
        // real `outermost` mode, no NEW match can legally begin there (that
        // is the whole premise `outermost` is named for), so the match()
        // attempt below is redundant work for every single event until that
        // capture closes. Skipping it here is the fast path being
        // benchmarked - see the file header comment for exactly what
        // skipMatchWhileCapturing does and doesn't model.
        if (this.skipMatchWhileCapturing && this.dispatcher) {
            return;
        }
